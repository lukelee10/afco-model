 ...
 */
public class ExtendedHeaderInfo19511 extends SupportedExtendedHeaderInfo {

    private static final Logger log = LoggerFactory.getLogger(ExtendedHeaderInfo19511.class);

    private final byte EXT_HEADER_19511_SPARE_BYTE = 0;
    /**
     * 64 chars
     */
    ...
    /**
     * 4 byte unsigned int represented as a long.
     */
    private long specialField1;

    /**
     * 15 character space padded string
     */
    private String specialField2;

    /**
     * Used if an attempt to salvage unsupported profile value comes in
     */
    private boolean profileOverride = false;

    /**
     * The value of the unsupported profile.  Current latest default supported is 19511
     */
    private int unsupportedProfileValue;

    @Override
    public int getAgsExtHeaderPreambleProfileValue() {
        if (!profileOverride) {
            return ExtendedHeaderInfo.AGS_EXTENDED_HEADER_PROFILE_19511_PREAMBLE_VALUE;
        }
        return unsupportedProfileValue;
    }

    /**
     * Overrides the latest profile with the parsed unsupportedProfileValue
     * @param unsupportedProfileValue
     * @return
     */
    public ExtendedHeaderInfo19511 unsupportedProfileOverride(int unsupportedProfileValue) {
        profileOverride = true;
        this.unsupportedProfileValue = unsupportedProfileValue;
        return this;
    }
    ...
    public byte[] toBytes() {
        ...
        try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
            ...
            // all chars before.
            bos.write(ExtendedHeaderInfo19511.uIntLongToByteWord(this.frequency));
            ...
            // Note: Extended header 19511's definition includes a spare byte from byte index 379 to 380.  Padding the
            // header with a single byte of value 0.
            bos.write(EXT_HEADER_19511_SPARE_BYTE);
            // Note the conversion to big-endian on this long -- left most bits are most significant in big-endian
            // Explanation:  The 32-bit integer will need to come right after all the previous bytes that happen to be
            // all chars before.
            bos.write(ExtendedHeaderInfo19511.uIntLongToByteWord(this.specialField1));
            // Write SpecialField2 bytes to extendedHeader
            bos.write(this.specialField2.getBytes(UTF8));
            // Note: Extended header 19511's definition includes a spare byte from byte index 399 to 400.  Padding the
            // header with a single byte of value 0.
            bos.write(EXT_HEADER_19511_SPARE_BYTE);
            ...
            if (lenPreLengthFixation != this.getActualAgsExtHeaderLengthInBytes() && !profileOverride) {
    ...
    public static ExtendedHeaderInfo19511 toExtendedHeaderFromBytes(byte[] rawExtendedHeader) {
        return toExtendedHeaderFromBytes(rawExtendedHeader, false);
    }
     ...
     * @return ExtendedHeaderInfo19511
     */
    public static ExtendedHeaderInfo19511 toExtendedHeaderFromBytes(byte[] rawExtendedHeader, boolean override) {
        log.trace("deserializing raw extension header bytes into ExtendedHeaderInfo instance...");
        if (!override) {
            if (rawExtendedHeader.length
                    != ExtendedHeaderInfo19511.getSupportedProfileBytesLength(
            ...
            log.warn(String.format(
                    "Warning, overriding extendedHeaderFromBytes incoming byte header [%d], expected byte header [%d]",
                    rawExtendedHeader.length,
                    ExtendedHeaderInfo19511.getSupportedProfileBytesLength(
                            ExtendedHeaderInfo.AGS_EXTENDED_HEADER_PROFILE_19511_PREAMBLE_VALUE)));
        }

        ExtendedHeaderInfo19511 tmp = new ExtendedHeaderInfo19511();
        ...
        tmp.setFrequency(ExtendedHeaderInfo19511.bytesToUIntLong(rawFrequency, 0));
        ...
        tmp.setSignalType(ExtendedHeaderInfo19511.signedByteToShort(rawSignalType[0]));
        // NOTE!! -- A spare byte exists in byte location 379. This value is not read
        //        log.debug("signalType [" + tmp.getSignalType()+"]");
        byte[] rawSpecialField1 = Arrays.copyOfRange(rawExtendedHeader, 380, 384);
        tmp.setSpecialField1(ExtendedHeaderInfo19511.bytesToUIntLong(rawSpecialField1, 0));
        //        log.debug("specialField1 [" + tmp.getSpecialField1()+"]");
        byte[] rawSpecialField2 = Arrays.copyOfRange(rawExtendedHeader, 384, 399);
        tmp.setSpecialField2(new String(rawSpecialField2));
        //        log.debug("specialField2 [" + tmp.getSpecialField2()+"]");
        // NOTE!! -- A spare byte exists in byte location 399. This value is not read
        return tmp;
    }
    ...
    public String toString() {
        ...
        buff.append(String.format("\nspecialField1 [%d]", this.getSpecialField1()));
        buff.append(String.format("\nspecialField2 [%s]", this.getSpecialField2()));
